...
- **Portfolio Categories**: `portfolio_photos.category` field (`people` or `spaces`) enables a People/Spaces toggle on the `/portfolio` page. Admin panel includes category filtering, category-aware uploads, and category editing per photo. People grid uses 3-column masonry layout (`columns-2 sm:columns-3`).
- **Before & After**: `portfolio_photos.before_image_url` stores a "before" headshot for comparison. Admin can upload via the tag editor modal. Portfolio page shows a draggable before/after slider section (only visible when photos have before images). Upload endpoint: `POST /api/admin/portfolio/:id/before-image`.
...
- **Space Favorites**: `space_favorites` table lets authenticated users favorite/unfavorite spaces. Heart icon on browse cards and space detail pages. Client Portal Spaces section has three sub-tabs: Favorites, My Spaces, and Past Spaces.
